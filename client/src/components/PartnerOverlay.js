...
class PartnerOverlay extends Component {
    ...
    render() {
        if (this.props.overlayVisibility) {
            return (
                <section data-id="margin" className={`${this.props.overlayPositioning} flex items-center justify-center top-0 left-0 h-full w-full pb-24`} onClick={this.props.closeOverlay}>
                    <div className="flex flex-col md:flex-row h-11/12 w-full sm:rounded sm:w-11/12 bg-gray-400 opacity-100">
                        <div className="flex flex-col flex-1 px-4 pb-4 md:pt-4 border-b-2 md:border-0 overflow-y-auto">
